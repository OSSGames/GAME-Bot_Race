...
void findTranslations( QTranslator *translator )
{
    ...
    if( !foundTranslation ) {
        // check local dir
        foundTranslation = translator->load( transFile,
                                             QString( "%1/translations" ).arg( QDir::currentPath() ) );
    }
...
{
    QCoreApplication::setOrganizationName( "BotRace" );
    QCoreApplication::setApplicationName( "BotRace" );
